...
// Цикл позволяет автоматизировать повторяющиеся задачи


// Допустим 10 повторений упражнений.
...
// Цикл for состоит из 3 параметров.
// 1 параметр - счётчик
// 2 условия, которые будут проверятся для очередного запуска цикла. Т.е. если значение выражения равно ИСТИНЕ то только тогда будет выполнятся код внутри тела
// 3 Уменьшение или увеличение счётчика. (переменная = переменная + значение)
// 4 НИКОГДА НЕ ПИШИ В КАЧЕСТВЕ ТРЕТЬЕГО ПАРАМЕТРА( ЗНАЧЕНИЕ = ЗНАЧЕНИЕ ++)
// Цикл работает следующий образом. Идет обращение к переменной (rep = 1), затем проверяется логическое значение (rep <= 10) и после этого идет выполнение ТЕЛА ЦИКЛА (то что находится в {})
// И только после этого запускается третье значение, которое работает до тех пор, пока значение второй переменной не станет false
// Ещё один вариант записи третьего свойства : rep += значение
// for(let rep = 0; rep <= 30; rep = rep +4.5 * 3) {
...
// console.log(user123[1]);
// console.log(user123[2]);
// console.log(user123[3]);
// console.log(user123[4]);
// console.log(user123[5]);

// В данном варианте использования цикла и массива, вывод данных из массива жёстко ограничен числом 4. 
// for(let i = 0; i <=4; i++) {
//    console.log(user123[i]);
// }

// В данном же варианте, вывод данных из массива будет равен длинне массива
// for(let i = 0; i < user123.length; i++) {
//    console.log(user123[i], typeof user123[i]); // Считываем данные из массива. В первом варианте выводит все значения из массива. Во втором узнаем их тип.
// };


// Выводим данные из массива в другой массив, на основе предыдущих действий с циклом и массивом. 
// В новый массив выводим тип данных массива пользователя. 
// for(let i = 0; i < user123.length; i++) {
//    console.log(user123[i], typeof user123[i]); 

   // types[i] = typeof user123[i]; // Простой способ получения типа элемента
   // types.push(typeof user123[i]); // Возвращает в нужном порядке
   // types.unshift(typeof user123[i]); // Возвращает в обратном порядке
// };
// console.log(types);
...
// const birthYear = [1974, 1994, 2000, 2003, 2000, 1932];
// const ages = [];

// for (let i = 0; i < birthYear.length; i++){

      // Первый вариант решения. 
      // const age = 2022 - birthYear[i];
      // ages.push(age)
...
//!========================================================================================================================================================
//? Break & Continue
// Два стейтмента для цикла.
// Break = остановить, сломать
// Continue = продолжить
...
// for(let i = 0; i < user123.length; i++) {
//    if (typeof user123[i] !== 'string') continue; // Вывод в консоль только строк. Следующим образом: ЕСЛИ тип элемента МАССИВА (user123) НЕ равен СТРОКЕ, то далее благодаря 'continue' мы грубо говоря продолжаем вывод данных только для тех, у которых есть 'stroke'
//    console.log(user123[i], typeof user123[i]); 
// };

// for(let i = 0; i < user123.length; i++) {
//    if (typeof user123[i] === 'number') break; // Мы сразу же выходит из цикла, как только встречаем элемент с указанным типом данных. Следовательно и дальнейший вывод в консоль прекращается.
//    console.log(user123[i], typeof user123[i]); 
// };
//!========================================================================================================================================================
...
//!========================================================================================================================================================
//? Циклы. Задание.
// Необходимо создать массивы для счетов, чаевых, итоговой суммы чаевых.
// Вычислить сумму чаевых на основе данных массива по формуле sum < 20 * 20% если же > 20 то 15%
// Используя функцию calculateTips() высчитать процент и с помощью цыкла заполнить два массива. Сумму чаевых и итоговую сумму (tips; totals)
// Затем найти среднее значение для итоговой суммы. 
// Создать функцию, которая будет подсчитывать среднее значение любого массива.
...
// Стрелочная функция, которая рассчитывает процент чаевых
const calculateTips = bill => bill < 20 ? bill * 0.2 : bill * 0.15; // Стрелочная функция, которая высчитывает процент надбавки на основе cчета. (Вместо bill можно использовать любые значения (a, b, c, d))

// Создание цикла, который считает процент для каждого элемента массива и возвращает данные в массив суммы чаевых.
// Цикл возвращает значения итоговой суммы массива, которое равно сумма массива + чаевые.
...
// Вывод в консоль
console.log(`Итого чаевых: ${tips} `);
console.log(`Итого сумма чека ${totals}`);

// Подсчёт суммы и среднего значения
const sum = totals.map(i=>x+=i, x=0).reverse()[0]; // Сумма
const arr = Math.trunc(sum) / totals.length; // Среднее значение

// Функция которая высчитывает среднее значение массива на основе его длинны.
const calculateAverage = function(arr) { // Вместо arr можно использовать любой символ или слово
   let sum = 0; // Переменная
   for (let i = 0; i < arr.length; i++) { // Параметры цикла
     sum = Math.trunc(sum) + arr[i]; // Подсчёт суммы и обнуление десятичные частей
   }
   return Math.trunc(sum) / arr.length; // Возвращаем среднее значение массива. Сумма делится на длинну массива
}

 // Выводы в консоль
console.log(`Средний доход равен ${arr}$`); // Средняя прибыль на основе чека и чаевых.
console.log(calculateAverage([1, 2, 3])); // Среднее между заданным массивом
console.log(calculateAverage(totals)); // Среднее между итоговой суммой
console.log(calculateAverage(bills)); // Среднее между суммой чека
console.log(calculateAverage(tips)); // Среднее между суммой чаевых
//!========================================================================================================================================================
