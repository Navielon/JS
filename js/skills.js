...
const calculateHumiditiesAmplitude = function (humidities) {
   ...
   for (let i = 0; i < humidities.length; i++){ // Отсчёт начинается с 0 индекса элемента, т.е. с первого элемента, и продолжается до концы длинны массива увеличиваясь на единицу
   ...
   if (currentHumidy > max){ // Если значение массива больше переменной MAX, то переменная max равна текущему значению, которое проверяется.
      // Т.е. 1 > 0 - Да. Значение max становится 1.  100 > 1 - Да. Значит max становится 100.  99 > 100 - Нет. Значит значение max остаётся 100. 
      max = currentHumidy;
   }
   if (currentHumidy < min){ // Если значение массива меньше переменной MIN, то переменная min равна текущему значению, которое проверяется.
      // Т.е. 32 < 64 - Да. Значение min становится 32. 12 < 32 - Да. Значение min становится 12. 16 < 12 - Нет. Значит значение min остаётся 12.
      min = currentHumidy;
   }
}
   console.log(min, max); // Выводим в консоль минимальное и максимальное значение цикла.
   return  max - min; // Возвращает амплитуду влажности (Максимальное значение минус минимальное)
};

const amplitude = calculateHumiditiesAmplitude(humidities); // Присваевает более короткое имя для вызова функции
console.log(amplitude); // Вывод в консоль результат функции